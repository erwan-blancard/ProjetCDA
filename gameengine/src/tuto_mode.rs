...
use crate::play_info::{PlayInfo, PlayAction, ActionTarget, ActionType};

pub fn test() {
    let mut game = Game::new(vec![
            Player::new("Joueur".to_string()),
            Player::new("Adversaire".to_string())
        ]
    );
    ...
    test_random(&mut game);
}

pub fn test_random(game: &mut Game) {
...
fn opponent_turn(opponent: &mut Player, player: &mut Player, deck: &mut Vec<Card>) -> bool {
    ...
    opponent.health <= 0 || player.health <= 0
}
...
fn play_card(card: &Card, player: &mut Player, opponent: &mut Player, deck: &mut Vec<Card>) {
    ...
    println!("Vie de {}: {}", player.name, player.health);
    println!("Vie de {}: {}", opponent.name, opponent.health);
...
let action = ActionType::Attack { amount: card.attack as u32};
