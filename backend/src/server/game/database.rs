...
struct BasicCardData {
    ...
    targets: TargetType,
    #[serde(default)]
    complex_effects: Vec<crate::server::game::card::ComplexEffect>,
}
    ...
    fn make_card(&self) -> Box<dyn Card> {
        match &self.variant {
            CardVariant::BasicCard(data) => {
                if !data.complex_effects.is_empty() {
                    Box::new(crate::server::game::card::ComplexEffectCard {
                        base: BasicCard {
                            id: self.id,
                        ...
                        complex_effects: data.complex_effects.clone(),
                    })
                } else {
                    Box::new(BasicCard {
                        id: self.id,
