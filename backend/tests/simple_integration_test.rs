...
#[actix_web::test]
async fn test_actix_web_get_integration() {
...
#[actix_web::test]
async fn test_actix_web_get_with_param() {
    let app = test::init_service(
        App::new()
            .service(test_route_with_param)
    ).await;

    // Test de la route GET avec paramètre
    let req = test::TestRequest::get()
        .uri("/test/John")
        .to_request();
    let resp = test::call_service(&app, req).await;
    ...
    let body = test::read_body(resp).await;
    let body_str = std::str::from_utf8(&body).unwrap();
    assert!(body_str.contains("Hello John from test route!"));
}

#[actix_web::test]
async fn test_actix_web_post_integration() {
    let app = test::init_service(
        App::new()
...
#[actix_web::test]
async fn test_multiple_routes_integration() {
    let app = test::init_service(
        App::new()
            .service(test_route)
            .service(test_route_with_param)
    ...
    // Test que toutes les routes fonctionnent ensemble
    let req1 = test::TestRequest::get().uri("/test").to_request();
    let resp1 = test::call_service(&app, req1).await;
    assert!(resp1.status().is_success());

    let req2 = test::TestRequest::get().uri("/test/Alice").to_request();
    let resp2 = test::call_service(&app, req2).await;
    assert!(resp2.status().is_success());

    let req3 = test::TestRequest::post().uri("/test").to_request();
